...
export function Step6Summary({ onEdit }: Step6SummaryProps) {
  ...
  const formatLanguages = () => {
    ...
    const niveaux = values.niveaux ?? {};
    return langues.map((lang) => `${lang} (${niveaux[lang] ?? 'N/A'})`).join(', ');
  };
  ...
  const sections = [
    ...
    },
    {
      ...
      fields: [
        ...
        { label: t('fields.consentement'), value: values.consentement ? 'Oui' : 'Non' },
      ],
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-foreground mb-2">{t('stepper.step6')}</h2>
        <p className="text-muted-foreground">Vérifiez vos informations avant de soumettre votre candidature.</p>
      </div>
        ...
        {sections.map((section) => (
          <Card key={section.step} className="relative">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Button variant="outline" size="sm" onClick={() => onEdit(section.step)}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {section.fields.map((field) => (
                  <div key={field.label} className="space-y-1">
                    ...
                    <dd className="text-sm text-foreground">{field.value || 'N/A'}</dd>
                  </div>
                ))}
              </div>
            </CardContent>
      ...
      <div className="bg-primary/10 border border-primary/20 rounded-lg p-4">
        <p className="text-sm font-medium text-primary mb-2">⚠️ Attention</p>
